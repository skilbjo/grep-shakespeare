var mark = '*',
	n = 25,
	create_arr = function(n){ var arr = []; for(var row=0; row < n; row++){var line = [];
			for(var field=0; field < n; field++){	line.push(' '); }	arr.push(line);	}	return arr;
	},
	arr = create_arr(n),
	textify = function(){	var result = ''; for(var row=0; row<= arr.length-1; row++){
		for(var col=0; col<= arr.length-1; col++){	result+=arr[row][col];	}	result+='\n';	}	return result;
	};
...
var directions_stack = function(n){
	var order = ['R','U','L','D'],
		directions = []
		;
...
var spiral = function(n){
	var next = draw_line([0,0],arr.length-1,'R'),
		directions = directions_stack(n);

	for(var i = n - 1; i >= 0; i--){
		next = draw_line( next , i, directions.pop() );
	}
};
